...
local function Refresh()
	...
	for k, v in pairs( team.GetPlayers(TEAM_DEATH) ) do
		if not v:Alive() or v:GetNWBool("Set_GhostMode", true) then pool[#pool+1] = v continue end
		CreatePlayer(v)
	end
	...
	if #pool > 0 then
		...
		for k, v in pairs( pool ) do
			if not IsValid(v) or v:GetNWBool("Set_GhostMode", true) then continue end
			CreatePlayer(v)
		end
	end
	
	CreateEmpty( 10 )
	CreateTeamThing( "Ghosts", Color( 238, 238, 238, 200 ) )
	

	for k, v in pairs( player.GetAll() ) do
		if not v:GetNWBool("Set_GhostMode", true) then pool[#pool+1] = v continue end
		CreatePlayer(v)
	end
...
-- tweaked by Dogey and xbeastguyx
